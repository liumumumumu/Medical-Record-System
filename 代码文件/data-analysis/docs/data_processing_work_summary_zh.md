...
1. 当前 `scripts/preprocess.py`、`scripts/feature_builder.py` 和 NHANES 相关脚本仍属于离线数据处理流程，尚未接入 `React -> Spring Boot -> Flask -> MongoDB` 在线链路。是否要求每次在线提交都经过该模块，需要与 C 和老师进一步确认。
2. 症状词典、医学术语词典和同义词表当前由数据处理模块维护。后续若 A 侧也维护识别词典，需要全组确认统一来源、版本号和同步方式，避免资源逐渐不一致。
