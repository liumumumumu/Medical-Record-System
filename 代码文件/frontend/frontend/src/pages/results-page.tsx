...
import { downloadAttachment, downloadReport, getCase, isUnauthorized, updateCase } from "../services/medical-api";
...
export function ResultsPage({ isLoggedIn, onAuthExpired, onRequireLogin }: ResultsPageProps) {
  ...
  async function handleAttachmentDownload(attachment: { id: string; fileName: string; url: string }) {
    ...
    try {
      ...
      if (isUnauthorized(requestError)) onAuthExpired();
      else setError(requestError instanceof Error ? requestError.message : "附件下载失败，请稍后重试。");
    } finally {
  ...
  return (
    <main className="results-main">
      ...
      <div className="results-document">
        ...
        <section className="result-section"><div className="result-section__index">04</div><div className="result-section__content"><p className="section-kicker">Attachments</p><h3>检查资料</h3>{attachments.length ? <div className="attachment-list">{attachments.map((attachment) => <div className="attachment-row" key={attachment.id}><FileTextOutlined /><span>{attachment.fileName}</span><button className="attachment-download" type="button" disabled={downloadingAttachmentId === attachment.id} onClick={() => { void handleAttachmentDownload(attachment); }}>{downloadingAttachmentId === attachment.id ? "下载中" : "下载"}</button><small>{attachmentStatusLabels[attachment.processingStatus] ?? attachment.processingStatus}{attachment.failureReason ? `：${attachment.failureReason}` : ""}</small>{attachment.extractedText ? <details><summary>查看提取文本</summary><p>{attachment.extractedText}</p></details> : null}</div>)}</div> : <p className="result-muted">当前病例未上传检查资料。</p>}</div></section>
      </div>
    </main>
  );
}
